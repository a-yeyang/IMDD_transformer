...
CONFIG = {
    ...
    'num_lstm_layers': 1,    # LSTM 层数
    'use_center': True,      # True: 取中心时刻输出; False: 取最后时刻
    'batch_size': 256,
    'epochs': 50,
    'lr': 0.001,
    'label_scale': 3.0,
...
class BiLSTMEqualizer(nn.Module):
    """
    双向 LSTM 均衡器。
    利用序列前后文信息进行均衡，取中心时刻的双向隐状态拼接后回归。
    """

    def __init__(self, input_size=1, hidden_size=16, num_layers=1,
                 use_center=True, window_size=21):
        super().__init__()
    ...
    def forward(self, src):
        # src: (batch, seq_len, 1)
        output, _ = self.lstm(src)  # (batch, seq_len, hidden*2)

        if self.use_center:
            x = output[:, self.center_idx, :]
        else:
            x = output[:, -1, :]
...
def train():
    ...
    log_path  = LOGS_DIR / f'bilstm_train_{timestamp}.log'
    log = logging.getLogger('bilstm_trainer')
    ...
    log.info("=" * 60)
    log.info("  BiLSTM Equalizer — 训练开始")
    log.info("=" * 60)
    ...
    for epoch in range(1, CONFIG['epochs'] + 1):
        ...
        if epoch % CONFIG['eval_interval'] == 0:
            ...
            if val_loss < best_val_loss:
                ...
                best_epoch    = epoch
                save_path = MODELS_DIR / 'bilstm_model.pth'
                torch.save({
    ...
    log.info(f"模型已保存至:  {MODELS_DIR / 'bilstm_model.pth'}")
    log.info("=" * 60)
    ...
    plt.suptitle(f'BiLSTM — Training Summary (Best BER={best_val_ber:.4e})', fontsize=12)
    plt.tight_layout()
    fig_path = IMAGES_DIR / 'bilstm_training_loss.png'
