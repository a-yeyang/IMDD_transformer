...
CONFIG = {
    ...
    'd_model': 16,
    'nhead': 2,
    'dim_feedforward': 16,   # KAN 非线性强，中间维度可更小
    'num_layers': 1,
    'num_passes': 2,
    'use_weight_sharing': True,
    'use_center_token': True,
    'use_sinusoidal_pe': True,
    'kan_grid_size': 3,      # 傅里叶基阶数，控制非线性复杂度
    'batch_size': 256,
    'epochs': 50,
    'lr': 0.001,
    'label_scale': 3.0,
...
class FastKANLinear(nn.Module):
    """
    轻量化 KAN：基础线性映射 + SiLU + 可学习傅里叶多项式基。
    比原版 B-Spline 实现更适合低延迟光通信 DSP 部署。
    """
    ...
    def __init__(self, in_features, out_features, grid_size=3):
        ...
        self.spline_weight = nn.Parameter(
            torch.randn(out_features, in_features, grid_size) / (in_features * grid_size)
        )

    def forward(self, x):
        base_output = self.base_linear(self.base_activation(x))

        # 傅里叶基：对周期性光器件响应有良好表达能力
        basis = torch.stack([torch.sin(i * x) for i in range(1, self.grid_size + 1)], dim=-1)
        spline_output = torch.einsum('bsig,oig->bso', basis, self.spline_weight)

        return base_output + spline_output


# ================= KAN-Transformer 编码器层 =================
    ...
    def __init__(self, input_dim, d_model, nhead, num_layers, dim_feedforward,
        ...
        if use_weight_sharing:
            ...
            self.layers = nn.ModuleList([
                KANTransformerEncoderLayer(d_model, nhead, dim_feedforward, grid_size=grid_size)
...
def train():
    ...
    log_path  = LOGS_DIR / f'kan_teq_train_{timestamp}.log'
    log = logging.getLogger('kan_trainer')
    ...
    log.info("=" * 60)
    log.info("  KAN-Former Equalizer — 训练开始")
    log.info("=" * 60)
    ...
    log.info(f"模型已保存至:  {MODELS_DIR / 'kan_teq_model.pth'}")
    log.info("=" * 60)
    ...
    plt.suptitle(f'KAN-Former — Training Summary (Best BER={best_val_ber:.4e})', fontsize=12)
    plt.tight_layout()
    fig_path = IMAGES_DIR / 'kan_teq_training_loss.png'
