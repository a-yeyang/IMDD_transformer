...
def setup_logger():
    ...
    log_path  = LOGS_DIR / f'compare_equalizers_{timestamp}.log'
    log = logging.getLogger('compare_equalizers')
...
def test():
    ...
    log.info("=" * 72)
    log.info("   四种均衡器综合对比 — BER vs SNR (无 Transformer)")
    log.info("=" * 72)
    ...
    # ---------- 保存 CSV ----------
    csv_path = ROOT / 'equalizer_comparison_results.csv'
    ...
    ax.set_title('IM/DD PAM4 均衡器对比 — BER vs SNR (FCNN / DNN / BiLSTM / RKAN)', fontsize=13)
    ax.legend(fontsize=10, loc='upper right')
    ax.grid(True, which='both', linestyle='--', alpha=0.6)

    plt.tight_layout()
    out_fig = IMAGES_DIR / 'equalizer_comparison_ber.png'
