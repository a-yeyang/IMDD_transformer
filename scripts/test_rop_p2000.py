"""
ROP 泛化实验 — ~2000 参数量级消融测试（CPU 多进程并行）

加载 train_all_rop_p2000.py 训练的模型，增量保存
  ROP测试数据/rop_ber_comparison_p2000.csv 与对应 PNG。

运行方式:
  python test_rop_p2000.py
  python test_rop_p2000.py --models ResKAN ResFCNN
  python test_rop_p2000.py --list
  python test_rop_p2000.py --plot-only
"""
...
from train_all_rop_p2000 import (
    FCNN_P2000, DNN_P2000, BILSTM_P2000,
    VANILLA_KAN_P2000, RES_KAN_P2000, RES_FCNN_P2000,
)
...
MODEL_REGISTRY = [
    {'name': 'FCNN', 'build': _build_fcnn, 'default_cfg': dict(FCNN_P2000),
     'ckpt': 'fcnn_p2000_rop_model.pth',
     'color': 'C2', 'marker': '^', 'ls': '-.'},
    {'name': 'DNN', 'build': _build_dnn, 'default_cfg': dict(DNN_P2000),
     'ckpt': 'dnn_p2000_rop_model.pth',
     'color': 'C4', 'marker': 'v', 'ls': '-.'},
    {'name': 'BiLSTM', 'build': _build_bilstm, 'default_cfg': dict(BILSTM_P2000),
     'ckpt': 'bilstm_p2000_rop_model.pth',
     'color': 'C3', 'marker': 'D', 'ls': ':'},
    {'name': 'VanillaKAN', 'build': _build_vanilla_kan,
     'default_cfg': dict(VANILLA_KAN_P2000),
     'ckpt': 'vanilla_kan_p2000_rop_model.pth',
     'color': 'C9', 'marker': 'P', 'ls': '-'},
    {'name': 'ResKAN', 'build': _build_res_kan, 'default_cfg': dict(RES_KAN_P2000),
     'ckpt': 'res_kan_p2000_rop_model.pth',
     'color': 'C0', 'marker': 'o', 'ls': '-'},
    {'name': 'ResFCNN', 'build': _build_res_fcnn, 'default_cfg': dict(RES_FCNN_P2000),
     'ckpt': 'res_fcnn_p2000_rop_model.pth',
...
def _render_rop_plot(csv_path, rop_for_plot, fig_path):
    ...
    matplotlib.rcParams.update({
        ...
        'legend.fontsize': 11, 'axes.unicode_minus': False,
    })
    ...
    ax.set_title(f'BER vs. ROP — {_BUDGET_TAG} Params Ablation')
...
def _worker(args):
    ...
    for rop, mat_path in rop_data_paths.items():
        ...
        if np.iscomplexobj(rx):
            rx = np.abs(rx)
        ds = OpticalDataset(
            rx, sym, config['window_size'], config['sps'],
...
def main(plot_only=False, model_names=None):
    csv_path = ROP_DIR / 'rop_ber_comparison_p2000.csv'
    fig_path = ROP_DIR / 'rop_ber_comparison_p2000.png'
    ...
    print("=" * 65)
    print(f"  ROP 泛化 — {_BUDGET_TAG} 参数量级消融测试")
    ...
    candidates = ([e for e in MODEL_REGISTRY if e['name'] in model_names]
                    if model_names else list(MODEL_REGISTRY))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='ROP 泛化 — ~2000 参数量级消融测试',
        epilog='示例:\n'
               '  python test_rop_p2000.py\n'
               '  python test_rop_p2000.py --models ResKAN ResFCNN\n'
               '  python test_rop_p2000.py --list\n'
