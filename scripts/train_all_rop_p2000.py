"""
ROP 泛化实验 — ~2000 参数量级消融训练脚本

六种均衡器参数量统一到 ~2000 量级（与 p500 / p1000 相同模型族）：

  对照组: FCNN [44,22] ~1981 | DNN [42,21]+BN ~1975 | BiLSTM hs=13 ~1925
  实验组: VanillaKAN h=10 ~1980 | ResKAN main[12,8]+kan=8 ~1962
  消融:   ResFCNN main[12,8]+res[70] ~1989

运行方式:
  python train_all_rop_p2000.py
  python train_all_rop_p2000.py --models res_kan res_fcnn
  python train_all_rop_p2000.py --list
"""
...
FCNN_P2000 = {**_SHARED, 'hidden_dims': [44, 22]}
DNN_P2000 = {
    **_SHARED,
    'hidden_dims': [42, 21],
    'use_batchnorm': True,
...
MODEL_REGISTRY = [
    {'key': 'fcnn', 'name': 'FCNN', 'group': 'baseline',
     'build': _build_fcnn, 'cfg': dict(FCNN_P2000),
     'ckpt': 'fcnn_p2000_rop_model.pth', 'epochs': 30},
    {'key': 'dnn', 'name': 'DNN', 'group': 'baseline',
     'build': _build_dnn, 'cfg': dict(DNN_P2000),
     'ckpt': 'dnn_p2000_rop_model.pth', 'epochs': 30},
    {'key': 'bilstm', 'name': 'BiLSTM', 'group': 'baseline',
     'build': _build_bilstm, 'cfg': dict(BILSTM_P2000),
     'ckpt': 'bilstm_p2000_rop_model.pth', 'epochs': 80},
    {'key': 'vanilla_kan', 'name': 'VanillaKAN', 'group': 'kan',
     'build': _build_vanilla_kan, 'cfg': dict(VANILLA_KAN_P2000),
     'ckpt': 'vanilla_kan_p2000_rop_model.pth', 'epochs': 40},
    {'key': 'res_kan', 'name': 'ResKAN', 'group': 'kan',
     'build': _build_res_kan, 'cfg': dict(RES_KAN_P2000),
     'ckpt': 'res_kan_p2000_rop_model.pth', 'epochs': 35},
    {'key': 'res_fcnn', 'name': 'ResFCNN', 'group': 'ablation',
     'build': _build_res_fcnn, 'cfg': dict(RES_FCNN_P2000),
     'ckpt': 'res_fcnn_p2000_rop_model.pth', 'epochs': 35},
]
...
def main(keys=None):
    ...
    log = logging.getLogger('train_rop_p2000')
    log.setLevel(logging.INFO)
    log.handlers.clear()
    fh = logging.FileHandler(
        LOGS_DIR / f'train_rop_p2000_{timestamp}.log', encoding='utf-8')
    ...
    log.info("=" * 65)
    log.info("  ROP 泛化实验 — ~2000 参数量级消融训练")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='ROP 泛化 — ~2000 参数量级消融训练',
