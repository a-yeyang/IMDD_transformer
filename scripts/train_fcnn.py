...
# ================= FCNN 均衡器 =================
class FCNNEqualizer(nn.Module):
    """
    全连接神经网络 (FCNN) 均衡器。
    将滑窗信号展平后经 MLP 映射到均衡输出，是最经典的 NN 均衡器基准。
    """
...
def train():
    ...
    log_path  = LOGS_DIR / f'fcnn_train_{timestamp}.log'
    log = logging.getLogger('fcnn_trainer')
    ...
    for epoch in range(1, CONFIG['epochs'] + 1):
        ...
        if epoch % CONFIG['eval_interval'] == 0:
            ...
            if val_loss < best_val_loss:
                ...
                best_epoch    = epoch
                save_path = MODELS_DIR / 'fcnn_model.pth'
                torch.save({
    ...
    log.info(f"模型已保存至:  {MODELS_DIR / 'fcnn_model.pth'}")
    log.info("=" * 60)
    ...
    plt.suptitle(f'FCNN — Training Summary (Best BER={best_val_ber:.4e})', fontsize=12)
    plt.tight_layout()
    fig_path = IMAGES_DIR / 'fcnn_training_loss.png'
